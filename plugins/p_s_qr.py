...
async def my_plan(client: Client, query: CallbackQuery):
    try:
        ...
        logger.error("🚨 [CALLBACK ERROR in p_s_qr.py]: my_plan failed", exc_info=True)
...
async def buy_qr(client: Client, query: CallbackQuery):
    try:
        ...
        logger.error("🚨 [CALLBACK ERROR in p_s_qr.py]: buy_qr failed", exc_info=True)
...
async def handle_ss(client: Client, message: Message):
    try:
        ...
        buttons = InlineKeyboardMarkup(
            [[InlineKeyboardButton("✅ Approve 30 Days", callback_data=f"app#{user_id}")]]
        )
        ...
        logger.error("🚨 [CRITICAL INPUT ERROR in p_s_qr.py]: handle_ss failed", exc_info=True)
...
async def approve_prem(client: Client, query: CallbackQuery):
    try:
        ...
        logger.error("🚨 [CALLBACK ERROR in p_s_qr.py]: approve_prem failed", exc_info=True)
