...
async def get_settings(chat_id: int):
    try:
        ...
        if not current:
            new_settings = {"chat_id": chat_id, **DEFAULT_SETTINGS}
            await settings_col.insert_one(new_settings)
            return new_settings
        return current
    except Exception as e:
        logger.error(f"Error fetching settings for chat {chat_id}: {e}", exc_info=True)
...
async def open_settings(client: Client, message: Message):
    try:
        ...
        logger.error("🚨 [CRITICAL COMMAND ERROR]: /settings failed", exc_info=True)
...
async def toggle_setting_callback(client: Client, query: CallbackQuery):
    try:
        ...
        await settings_col.update_one({"chat_id": chat_id}, {"$set": {key: new_val}}, upsert=True)
        ...
        logger.error("🚨 [CALLBACK ERROR]: toggle setting failed", exc_info=True)
        await query.answer("⚠️ Failed to update setting.", show_alert=True)


@Client.on_callback_query(filters.regex(r"^set_caption_menu$"))
async def caption_menu(client: Client, query: CallbackQuery):
    try:
        ...
        logger.error("🚨 [CALLBACK ERROR]: caption_menu failed", exc_info=True)
        await query.answer("⚠️ Failed to open caption settings.", show_alert=True)


@Client.on_callback_query(filters.regex(r"^set_tutorial_menu$"))
async def tutorial_menu(client: Client, query: CallbackQuery):
    try:
        ...
        logger.error("🚨 [CALLBACK ERROR]: tutorial_menu failed", exc_info=True)
        await query.answer("⚠️ Failed to open tutorial settings.", show_alert=True)


@Client.on_callback_query(filters.regex(r"^set_movie_req$"))
async def movie_req_menu(client: Client, query: CallbackQuery):
    try:
        ...
        logger.error("🚨 [CALLBACK ERROR]: movie_req_menu failed", exc_info=True)
        await query.answer("⚠️ Failed to open movie request settings.", show_alert=True)


@Client.on_callback_query(filters.regex(r"^settings_details$"))
async def settings_details_callback(client: Client, query: CallbackQuery):
    try:
        ...
        logger.error("🚨 [CALLBACK ERROR]: settings_details failed", exc_info=True)
        await query.answer("⚠️ Failed to load details.", show_alert=True)


@Client.on_callback_query(filters.regex(r"^set_max_results$"))
async def max_results_menu(client: Client, query: CallbackQuery):
    try:
        s = await get_settings(query.message.chat.id)
        text = (
            "ℹ️ **Max Results Per Page:**\n\n"
            f"Current limit: `{s.get('max_results', 10)}`\n\n"
        ...
        logger.error("🚨 [CALLBACK ERROR]: max_results_menu failed", exc_info=True)
        await query.answer("⚠️ Failed to open max results.", show_alert=True)
...
async def back_to_main_settings(client: Client, query: CallbackQuery):
    try:
        ...
        logger.error("🚨 [CALLBACK ERROR]: back_to_settings failed", exc_info=True)
        await query.answer("⚠️ Failed to return to settings.", show_alert=True)
    ...
    try:
        ...
        if update_field:
            await settings_col.update_one(
                {"chat_id": chat_id},
                {"$set": {update_field: val}},
                upsert=True,
            )
            await message.reply_text(f"✅ Successfully updated **{update_field}**!")

    except Exception:
        logger.error("🚨 [CRITICAL INPUT ERROR]: capture_group_settings_inputs failed", exc_info=True)
