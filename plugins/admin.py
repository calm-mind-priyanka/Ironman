...
async def add_premium_cmd(
    ...
    try:
        ...
        if message.from_user.id not in getattr(
            Config,
            "ADMINS",
            []
        ):
...
async def remove_premium_cmd(
    ...
    try:
        ...
        result = await premium_col.delete_one(
            {
                "user_id": uid
            }
        )
