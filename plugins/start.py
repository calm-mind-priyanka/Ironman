    ...
    try:
        ...
        buttons = [
            [
                ...
                InlineKeyboardButton("💎 My Plan", callback_data="my_plan_menu"),
            ],
...
async def help_menu(client: Client, query):
    try:
        text = (
            "📚 **Help Menu**\n\n"
        ...
        logger.exception("Error in help menu: %s", e)
        try:
            await query.answer("⚠️ Failed to open help.", show_alert=True)
        except Exception:
            pass
...
async def back_to_start(client: Client, query):
    try:
        ...
        logger.exception("Error returning to start menu: %s", e)
        try:
            await query.answer("⚠️ Failed to go back.", show_alert=True)
        except Exception:
            pass
